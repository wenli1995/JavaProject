...
public class BlogDao {
	...
	public List<Blog> queryOrderByCommentNum(Connection conn,int userId,int num) throws SQLException{
		ArrayList<Blog> blogList=new ArrayList<>();
		String sql="SELECT a.blogId,a.title,COUNT(commentId) commentnum FROM blog a ,COMMENT b WHERE a.blogId=b.blogId   AND a.userId=?   and status=1 GROUP BY a.blogId ORDER BY commentnum DESC LIMIT 0,?";
		PreparedStatement stat=conn.prepareStatement(sql);
